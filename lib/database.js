...
const papers = {
   ...
   * 論文一覧を取得
   */
  async findAll({ journalIds, dateFrom, dateTo, limit = 100, offset = 0, search } = {}) {
    let sql = `
      SELECT 
        p.*,
   ...
   * 論文数を取得
   */
  async count({ journalIds, dateFrom, dateTo, search } = {}) {
...
const users = {
  ...
  async create(user) {
    ...
    const result = await query(sql, [user.username, user.password_hash, user.email, user.is_admin || false]);
