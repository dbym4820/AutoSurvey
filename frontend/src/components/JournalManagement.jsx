...
export default function JournalManagement({ onBack }) {
  const [journals, setJournals] = useState([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingJournal, setEditingJournal] = useState(null);
  const [showInactive, setShowInactive] = useState(false);
  const [fetchingJournal, setFetchingJournal] = useState(null);
            ...
            {journals.map((journal) => (
              <div
                ...
                className={`bg-white rounded-xl shadow-sm border p-5 transition-opacity ${
                  journal.is_active === 0 ? 'opacity-60 border-gray-300' : 'border-gray-200'
                }`}
              >
